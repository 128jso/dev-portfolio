...
import { EXPERIENCE_TIMELINE } from '../data/githubData';
    ...
    <section id="experience" style={{ padding: '2.5rem 1.5rem', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <h2 style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>
          Work Experience & Engineering Journey
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
          Key roles, technical accomplishments, and architectural impact across projects.
        </p>
      </div>
      ...
      <div style={{ position: 'relative', paddingLeft: '1.5rem', borderLeft: '2px solid var(--border-subtle)' }}>
        ...
        {EXPERIENCE_TIMELINE.map((item, idx) => (
          <div key={item.id} style={{ position: 'relative', marginBottom: idx === EXPERIENCE_TIMELINE.length - 1 ? 0 : '2.25rem' }}>
            ...
            <div className="clean-panel" style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem', marginBottom: '0.65rem' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.92rem', fontWeight: 700, color: 'var(--accent-purple-light)' }}>{item.company}</div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', marginBottom: '1.1rem' }}>
                {item.bullets.map((b, bIdx) => (
                  <div key={bIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.45rem', fontSize: '0.88rem', color: 'var(--text-primary)' }}>
                    <CheckCircle2 size={15} color="var(--accent-purple-light)" style={{ flexShrink: 0, marginTop: '2px' }} />
                    <span>{b}</span>
                  </div>
                ))}
              </div>
